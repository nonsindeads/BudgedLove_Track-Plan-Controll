...
if (!empty($_SESSION['is_admin'])) {
    $navSections[count($navSections) - 1]['items'][] = ['key' => 'admin', 'label' => 'Admin', 'href' => '/admin.php', 'icon' => 'shield-lock'];
    $navSections[count($navSections) - 1]['items'][] = ['key' => 'translations', 'label' => 'Translations', 'href' => '/translations.php', 'icon' => 'translate'];
}
...
<aside class="hb-sidebar d-flex flex-column">
  <div class="hb-sidebar-header d-flex align-items-center gap-2 px-3 py-3 border-bottom">
    <div class="hb-logo d-flex align-items-center justify-content-center">
      <?= $hbSidebarLogoSvg; ?>
    </div>
  ...
  <nav class="flex-grow-1 py-3">
    <?php foreach ($navSections as $section): ?>
      <div class="hb-sidebar-section">
        <div class="hb-sidebar-section-title text-uppercase small"><?= htmlspecialchars(hb_t($section['title']), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?></div>
        <ul class="nav nav-pills flex-column">
          <?php foreach ($section['items'] as $item): ?>
            <?php $active = ($activeNav ?? '') === $item['key']; ?>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2 <?= $active ? 'active' : '' ?>" href="<?= htmlspecialchars($item['href'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>">
  ...
  <div class="hb-sidebar-footer border-top px-3 py-3">
    <?php if (!empty($currentUser)): ?>
      <div class="d-flex align-items-center mb-2 hb-sidebar-profile">
        <div class="hb-avatar me-2 text-white">
          <?= strtoupper(substr(($currentUser['first_name'] ?? $currentUser['username'] ?? '?'), 0, 1)) ?>
        </div>
        <div class="flex-grow-1 hb-sidebar-profile-text">
      ...
      <div class="d-flex flex-column gap-2 hb-sidebar-actions">
        <a class="btn btn-sm w-100 d-flex align-items-center justify-content-center <?= $profileActive ? 'btn-primary' : 'btn-outline-secondary' ?>" href="/profile.php" aria-label="<?= htmlspecialchars(hb_t('Profile'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>">
          <i class="bi bi-person-gear me-1"></i>
          <span class="hb-sidebar-action-text"><?= htmlspecialchars(hb_t('Profile'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?></span>
        </a>
        <form method="post" action="/auth.php?action=logout" class="d-grid">
          <?= hb_csrf_field() ?>
          <button type="submit" class="btn btn-outline-danger btn-sm w-100 d-flex align-items-center justify-content-center" aria-label="<?= htmlspecialchars(hb_t('Logout'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?>">
            <i class="bi bi-box-arrow-right me-1"></i>
            <span class="hb-sidebar-action-text"><?= htmlspecialchars(hb_t('Logout'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') ?></span>
          </button>
        </form>
      </div>
    <?php else: ?>
